...
- Fully compatible with Exa search results (long JSON / nested dicts)
- No duplicated helpers, no broken regex, no multi-line corruption
"""
...
# --------------------------------------------------------------------
# Regex Patterns (clean + simple)
# --------------------------------------------------------------------
HEADER_BULLET = re.compile(r'^(?:[-*\u2022]|\d+[.)])\s+#')
...
def _preclean_near_json(s: str) -> str:
    ...
    s = re.sub(r"```(?:json)?\s*(.*?)\s*```", r"\1", s, flags=re.I | re.S)
    # Remove bullets from lines
        ...
        if key in ("trends", "insights", "opportunities", "risks", "sources"):
            ...
            for i, (p, a, w) in enumerate(zip(prio, act, why), 1):
                merged["recommendations"].append(
                    {"priority": p or i, "action": a, "rationale": w}
                )
